...
void DisplayHelp() {
	...
	std::cout << "x: Read the current position using DK\n";
	...
	std::cout << "s: Set the crash limit for all motors seperately\n";
	std::cout << "S: Set the crash limit for all motors\n";
	std::cout << "t: Switch crash limit on\n";
	std::cout << "T: Switch crash limit off\n";
	std::cout << "u: Unblock motors after crash\n";
	std::cout << "d: Move motor to degrees\n\n\n";
//*******************************************************************************/
//test
	std::cout << "Key control of Katana, use the following keys:\n\n";
...
int main(int argc, char *argv[]) {
	...
	if (argc != 3) {
		std::cout << "---------------------------------\n";
		std::cout << "usage: keycontrol CONFIGFILE PORT\n";
		std::cout << "---------------------------------\n";
		return 0;
	}
	...
	std::cout << "-----------------------\n";
	std::cout << "KEYCONTROL DEMO STARTED\n";
	std::cout << "-----------------------\n";
	...
	*/
	...
	while (loop) {
		...
		try {
			...
			case 's': {
					int limit;
					std::cout << "Set crash limit for motors to: \n";
					for(short motorNumber = 0; motorNumber < katana->getNumberOfMotors(); ++motorNumber) {
						std::cout << motorNumber+1 << ": ";
						std::cin >> limit;
			...
			case 'S': {
					int limit;
					std::cout << "Set crash limit for all motors to: \n";
					std::cin >> limit;
					for(short motorNumber = 0; motorNumber < katana->getNumberOfMotors(); ++motorNumber) {
						katana->setCrashLimit(motorNumber, limit);
					}
...
void* RunProgram(void*){
	...
	while(progRunning){
		std::cout << " -> write your positions to use in the Tpos arrays and "
			<< "move the robot in the program loop." << std::endl;
		/*
